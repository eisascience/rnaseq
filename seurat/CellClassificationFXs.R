SaveDimRedux_SERIII <- function(seuratObj, reductions=c("pca", "tsne", "umap"),
  ...
  if(!is.na(save.path)){
    ...
    if(!is.na(save.path)){
      ...
      save.path <- paste(gsub(".csv", "", save.path), nameID, "_DimReduxComps.csv", sep="")
      write.csv(tempDT, file = save.path, row.names=TRUE, col.names = T)

    }


  } else {
    print("save path is NA")
    head(tempDT)
  }
      ...
      for(SERObj.path in SERObjects_processed.paths){
        ...
        if(ModuleScoreGeneListClassify){
          ...
          if(!is.na(yhat.save.path)){
            ...
            yhat.save.path <- paste(gsub(".csv", "", yhat.save.path), basename(SERObj.path), "_SGS.csv", sep="")
            write.csv(SGS.DF, file = yhat.save.path, row.names=TRUE, col.names = T)
        ...
        if(MCEClassify){
          ...
          if(!is.na(yhat.save.path)){
            ...
            write.csv(Classificatio.meta.data, file = yhat.save.path, row.names=TRUE, col.names = T)

            yhat.save.path <- paste(gsub(".csv", "", yhat.save.path), basename(SERObj.path), "_MCEyhat.csv", sep="")
            write.csv(yhat.Combo, file = yhat.save.path, row.names=TRUE, col.names = T)
