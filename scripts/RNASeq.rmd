...
resumeFile <- 'deseq2_vst.rds'
if (file.exists(resumeFile)){
	vst <- readRDS(resumeFile)
} else {
	vst <- vst(dds, blind=F)
	saveRDS(vst, resumeFile)
}
...
if(!(exists('skipDESeqWGCNA') && skipDESeqWGCNA)){
	save.image(file='checkpoint.rdata')
	allowWGCNAThreads()
	run_WGCNA_top(rld, 'wgcna_deseq.txt', as.integer(meta$GroupCol), levels(meta$GroupCol), top=5000)
} else {
  print('skipping edgeR wgcna')
}
