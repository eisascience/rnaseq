...
cex1 = 1.5;
plotPCA(rld, intgroup=c("GroupCol"))
dev.off()
...
#PCA
cex1 = 1.5;
plotPCA(vsd, intgroup=c("GroupCol"))
dev.off()
...
if(!(exists('skipDESeqWGCNA') && skipDESeqWGCNA)){
	save.image(file='checkpoint.rdata')
	allowWGCNAThreads()
	run_WGCNA_top_DESeq(vsd, 'wgcna_deseq.txt', as.integer(meta$GroupCol), levels(meta$GroupCol), top=5000)
} else {
