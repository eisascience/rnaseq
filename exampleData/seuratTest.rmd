...
for (datasetName in names(data)) {
  ...
  seuratObjs[datasetName] <- readAndFilter10xData(paste0(data[[datasetName]]), datasetName)
...
seuratObj <- processSeurat1(seuratObj, doCellCycle = F)
...
seuratObj <- findClustersAndRunTSNE(seuratObj, 1:dimsToUse, saveFile = saveFile)
...
findMarkers(seuratObj, resolutionToUse, outFile = paste0(outPrefix, '.', suffix, '.markers.txt'), saveFileMarkers = paste0(outPrefix, '.', suffix, '.markers.rds'))
